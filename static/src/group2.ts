...
export default {
  ...
  */
  rangeSumOfBst(root: Tree, L: number, R: number) {
    return totalUp(root, 0);

    function totalUp(root: Tree|null, total: number): number {
      if (root) {
  ...
  */
  maximumProductOfTwoElementsInArray(nums: number[]) {
  ...
  */
  studentsDoingHomeworkAtAGivenTime(
    startTime: number[],
    endTime: number[],
    queryTime: number
  ) {
  ...
  */
  minimumTimeVisitingAllPoints(points: number[][]) {
    let total = 0;
