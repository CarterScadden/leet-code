...
export default {
  ...
  */
  decompressRunLengthEncodedList(numbers: number[]) {
    let arr: number[] = [];
  ...
  createTargetInArray(nums: number[], indexs: number[]) {
    let arr: number[] = [];
    for (const i in nums) {
  ...
  */
  ...
  amountOfStringLengthOfNumbers(numbers: number[]) {
    let total = 0;
  ...
  jewelsAndStones(jewels: string, rocks: string) {
    let total = 0;
  ...
  kidsWithCandies(candies: number[], extraCandies: number) {
  ...
  numberSmallerThanGiven(numbers: number[]) {
    const arr = [];
    for (const num of numbers) {
      let total = 0;
  ...
  ShuffleTheArray(n: number, nums: number[]) {
    // [ [], [], [] ]
    let arr: number[][] = [];
  ...
  splitStringsInArray(str: string) {
    let countStr = 0;
    let r = 0;
    let l = 0;
  ...
  subtractTheProductAndSumOfDigitsOfInterger(num: number) {
    const strings = num.toString().split('');
    const numbers: number[] = [];

    for (const i in strings) numbers[i] = +strings[i];

    let mult = 1;
    for (const num of numbers) mult *= num;

    let add = 0;
    for (const num of numbers) add += num;
